...
The analysis was completed using the R and Python programming languages [R]; [Python]. The R packages used to to perform the analysis are tidyverse [@tidyverse], dplyr [@wickham2015dplyr], datateacher [@datateacher], infer [@infer], ggplot [@ggplot], purrr [@purrr], knitr [@knitr], and docopt [@docopt]. The Python packages used for the EDA and data wrangling scripts are pandas [@reback2020pandas], altair [@Altair_Developers], altair_saver [@altair_saver], pytest [@pytestx.y], and chromedriver_binary [@chromedriver_binary].
